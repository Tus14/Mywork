...
- **file_operations** - функции для дополнительной работы с транзакциями
...
**def process_bank_search** Фильтрует операции по строке поиска в описании с использованием регулярных выражений

- **тесты с использованием фикстур и параметризации**:
1. Фикстура с транзакциями для тестирования поиска по описанию
2. Тестирование поиска транзакций по описанию с использованием регулярных выражений
3. Тестирование поиска с пустым списком транзакций
4. Тестирование поиска, когда у некоторых транзакций нет поля description

**def process_bank_operations**Подсчитывает количество операций по заданным категориям.

-**тесты с использованием фикстур и параметризации**:
1. Фикстура с транзакциями для тестирования подсчета по категориям
2. Тестирование подсчета транзакций по категориям
3. Тестирование подсчета с пустым списком транзакций
4. Тестирование подсчета, когда у некоторых транзакций нет поля description
5. Тестирование регистронезависимости при подсчете категорий
...
-вложенный список
## Модуль main.py в корне проекта
- обеспечение проверки всех функций проекта с данными
- связь всего проекта по алгоритму: Программа-Пользователь
1. def main объединяет весь проект, где функции:
- load_json_file, load_csv_file и load_xlsx_file помогают
обрабатывать информацию
- process_transactions, _filter_by_category, _get_currency
_get_valid_statu, _ask_yes_no, _ask_choice, _print_results
_print_transaction, _format_date, _mask_account
помогают детализировать информацию для пользователя
